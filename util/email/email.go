...
func SendForgotEmail(email string, fullname string, verifyCode string) error {
	...
	m.SetHeader("Subject", "Welcome to myrepair")
...
// SendLogEmail sends log comments
func SendLogEmail(email string, joblog model.Joblog) error {
	fmt.Println(email, joblog.CustomerID)
	m := gomail.NewMessage()
	m.SetHeader("From", "skyclean906@gmail.com") // config.EmailUsername
	m.SetHeader("To", email)
	m.SetHeader("Subject", "Welcome to myrepair")
	...
	text := `<p>Price : <strong>` + strconv.Itoa(int(joblog.Price)) + `£` + `</strong></p>
			<p>Status : <strong>` + str_status + `</strong></p>
			<p>Log Comment : <strong>` + joblog.Comment + `</strong></p>
