...
func CreateUser(user *model.User) (*model.User, error) {
	...
	u := &model.User{}
	if res := db.ORM.Where("UserName = ?", user.UserName).First(&u).RecordNotFound(); !res {
		err := errors.New(user.UserName + " is already registered")
		return nil, err
	}
...
func ReadUsers(query string, offset int, count int, field string, sort int) ([]*model.User, int, error) {
	...
	if query != "" {
		query = "%" + query + "%"
		res = res.Where("UserName LIKE ? OR UserFirstName LIKE ? OR UserLastName LIKE ?", query, query, query)
	}
...
func ReadUserByUsername(username string) (*model.User, error) {
	user := &model.User{}
	res := db.ORM.Where("UserName = ?", username).First(&user).RecordNotFound()
	if res {
